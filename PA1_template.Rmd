...
The total number of steps taken per day is calculated and shown in the histogram below. The mean value of the total number of steps taken per day is `r mean_sumday` and highlighted by a vertical line in red. The median number of steps taken each day is `r median_sumday` and highlighted by a vertical line in blue. 
...
maxintervaltime
```
On the average across all the days in the dataset, the `r maxintervalseq`th 5-minute interval (`r maxintervaltime`) contains the maximum number of steps `r round(max(intervalave))`. 
...
median_sumnew
```
The total number of steps each day is shown in a histogram below. The mean value of total number of steps taken each day is `r mean_sumnew` and highlighted by a vertical line in red. The median value is `r median_sumnew` and highlighted by a vertical line in blue. The mean and the median are overlapped. Comparing to the original data set, both mean and median has increased. The mean value has increased more than the median value.  
...
By imputing the missing data, the total daily number of steps increases from `r totalsteps` to `r totalnew`. 
